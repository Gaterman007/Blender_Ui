...
class base_Element:
    ...
    def set_attribute(self, element, name, attr_type=str):
        ...
        if value is not None:
            if attr_type == float and isinstance(value, str):
                value = value.rstrip("mm").rstrip("cm").rstrip("in").rstrip("pt").rstrip("pc").rstrip("px").strip()           
            try:
    ...
    def from_json(self,element):
        ...
        if self["style"] is not None:
            style_pairs = self["style"].split(';')
            for pair in style_pairs:
                if ':' in pair:
                    name, value = pair.split(':', 1)  # Split au premier `:`
                    name = name.strip()
                    value = value.strip()
                    if name in self.attributes_to_get:
                        attr_type = self.attributes_to_get[name]
                        try:
                            value = attr_type(value)
                            self._attributes[name] = value
                        except ValueError:
